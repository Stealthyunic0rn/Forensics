...
This challenged comes with evidence from 4 machines and a pcap file from the 172.16.150.0/24 network.
...
## Memory Analysis
First I wanted to get as much information as possible on the differnet images. Using both **imageinfo** and the **connections** command gives me the profile, time the image was taken, and the IP of that NIC. 
...
The ports were 1097, 1098, 1099, 80, and other TCP ports. The 1097-99 ports are commnonly used for malware especially RATs.
...
## ENG-USTXHOU-148
Since we know that one of the IPs is one of our images I decided to dig a bit deeper in this specific memory dump. Since we already know the imageinfo I can just start **pstree**  right off the bat. The output looked pretty normal but what stood out to me was the times of the processes. 
...
Diggin into the **1024** process I wanted to check all the DLLs that are loaded on this guy. After alot of comparing and constrasting (thanks Google) a DLL named *6to4ex.dll** may be potentially malicious.
...
This system definetly got infected
I used **iehistory** and **cmdscan**. It looks like the user may have accidentaly clicked on a link that lead to a **Symantec-1.43-1.exe**. The output of cmdscan shows a user opening a share called *ITSHARE*on one of the servers (DC-USTXHOU) and performing a memory dump.
...
So we have command history, a malicious link but still no origin. I ran the strings command with that external C2 IP (58.64.132.141). I get what looks like an SMTP header and wanted to see if their was any context behind it. 
...
Lastly, I wanted the hash of the process 6to4ex. I used **procdump** to turn it into an executable and  checking the MD5 hash on VirusTotal returns back positive.
...
## FLD
We run **connscan** and notice the C2 server talking with this machine. The process assoicated with this is *1032, svchost.exe.
	
	0x02023638 172.16.223.187:1265       58.64.132.141:80          1032

Time to check the DLLs of this one. I did the same with the last machine and found our malicious DLL. I ran strings against the dumped module to check for "Gh0st" and got results. I also got back the same phising email which tells me that this machine also was infected.
...
Just out of curioustity I check the IE history of the machine and the results show that malicious link.	
	
	URL: amirs@http://58.64.132.8/download/Symantec-1.43-1.exe

## IIS 
The last machine to analyze and going through our routine their does not appear to be much. No sign of Gh0st, the email, or the C2 IP. Their is however, a process *PSEXESVC.exe** which is used fo rremotely executing processes on other machines. 

	.... 0x81f7f2b0:PSEXESVC.EXE                          268    528      4     85 2012-11-27 00:05:49 UTC+0000

This machine was probably targeted and files were taken from here, but it does not appear they machine itself is infected.

Just to recap, at this point we managed to find
1.  C2 server IP (58.64.122.141)
2. The malicous DLL 6to4ex.dll which was loaded into svchost.exe
3. The DLL MD5 hash  156f2c6a65a1eab1c03e1dc7f215a044  
4. The Gh0st string in the memory image
5. A potentialy malicious link /download/Symantec-1.43-1.exe
6. A phising email with the sender and recievers with the domain
